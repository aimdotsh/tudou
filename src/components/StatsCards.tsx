      ...
      <div className="col-span-2 sm:col-span-1 bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-4 sm:p-5 transition-all duration-300 hover:shadow-lg hover:shadow-[var(--color-accent)]/5 hover:border-[var(--color-accent)]/30 hover:bg-[var(--color-accent)]/5 min-w-0 overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <p className="text-2xl sm:text-3xl font-bold font-mono truncate">
            {currentStreak}
            <span className="text-sm sm:text-base font-normal text-[var(--color-muted)] ml-1">{t('days')}</span>
          </p>
          {/* Week streak badge on top right */}
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-500 shrink-0">
            ...
            <span className="text-xs font-bold font-mono">{currentWeekStreak}</span>
            <span className="text-[10px] opacity-80">{t('weeks')}</span>
          </div>
        </div>

        {/* Week days visual - Takes full width now */}
        {(() => {
            ...
            <div className="mt-3.5 flex items-center justify-between w-full">
              {weekDays.map((wd, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex flex-col items-center gap-1 ${wd.hasActivity ? 'cursor-pointer' : ''}`}
                    onClick={() => { if (wd.acts.length > 0) onSelectActivity(wd.acts[0]) }}
                  >
                    <span className="text-[10px] font-medium text-[var(--color-muted)]">{weekLabels[i]}</span>
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-medium transition-opacity ${
                        wd.hasActivity
                          ? 'text-white hover:opacity-70 shadow-sm'
                          : wd.isToday
                            ? 'ring-1.5 ring-[var(--color-text)] text-[var(--color-text)] font-bold'
                            : isPast
        ...
        <div className="mt-3.5 flex items-center gap-2 text-xs sm:text-sm text-[var(--color-muted)] pt-2 border-t border-[var(--color-border)]/40">
          <svg className="w-3.5 h-3.5 text-[var(--color-accent)] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
          </svg>
          <span className="truncate">{t('longest')}: {longestStreak} {t('days')} / {longestWeekStreak} {t('weeks')}</span>
        </div>
      </div>
    </div>
  )
