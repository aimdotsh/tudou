...
export function RouteMap({
  ...
  return (
    <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl overflow-hidden h-[280px] relative select-none shadow-sm transition-all duration-300">
      {/* 顶部重置 Overview 按钮 */}
      {selectedActivity && (
        <button
          onClick={onClearSelection}
          className="absolute top-3 left-3 z-20 px-3 py-1.5 bg-[var(--color-card)]/90 backdrop-blur-md border border-[var(--color-border)] rounded-lg text-xs font-medium text-[var(--color-text)] shadow-md hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] transition-all flex items-center gap-1.5"
        >
