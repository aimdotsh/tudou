...
import { Dumbbell, Flame, Timer, Activity, Sparkles, Droplets, ChevronLeft, ChevronRight, Clock } from 'lucide-react'
...
interface LogEntry {
  ...
  item: CheckinItem | 'coldShower'
  at: string          // ISO timestamp
  count?: number
  sets?: number
}

const ITEM_META: Record<CheckinItem | 'coldShower', {
  zh: string; en: string; color: string; bg: string; border: string; text: string;
  unitZh: string; unitEn: string;
  Icon: ({ className }: { className?: string }) => JSX.Element
}> = {
  pushups: {
    zh: '俯卧撑', en: 'Pushups',
    color: '#f97316', bg: 'bg-orange-500/10', border: 'border-orange-500/20', text: 'text-orange-500',
    unitZh: '个', unitEn: 'reps',
    Icon: ({ className }) => <Dumbbell className={className} />,
  },
  squats: {
    zh: '深蹲', en: 'Squats',
    color: '#3b82f6', bg: 'bg-blue-500/10', border: 'border-blue-500/20', text: 'text-blue-500',
    unitZh: '个', unitEn: 'reps',
    Icon: ({ className }) => <Dumbbell className={className} style={{ transform: 'rotate(90deg)' }} />,
  },
  pullups: {
    zh: '引体向上', en: 'Pull-ups',
    color: '#a855f7', bg: 'bg-purple-500/10', border: 'border-purple-500/20', text: 'text-purple-500',
    unitZh: '个', unitEn: 'reps',
    Icon: ({ className }) => <Flame className={className} />,
  },
  plank: {
    zh: '平板支撑', en: 'Plank',
    color: '#10b981', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-500',
    unitZh: '秒', unitEn: 's',
    Icon: ({ className }) => <Timer className={className} />,
  },
  kneeDrives: {
    zh: '提膝下压', en: 'Knee Drives',
    color: '#f43f5e', bg: 'bg-rose-500/10', border: 'border-rose-500/20', text: 'text-rose-500',
    unitZh: '个', unitEn: 'reps',
    Icon: ({ className }) => <Activity className={className} />,
  },
  underlegClaps: {
    zh: '胯下击掌', en: 'Underleg Claps',
    color: '#f59e0b', bg: 'bg-amber-500/10', border: 'border-amber-500/20', text: 'text-amber-500',
    unitZh: '个', unitEn: 'reps',
    Icon: ({ className }) => <Sparkles className={className} />,
  },
  coldShower: {
    zh: '冷水澡', en: 'Cold Shower',
    color: '#06b6d4', bg: 'bg-cyan-500/10', border: 'border-cyan-500/20', text: 'text-cyan-500',
    unitZh: '', unitEn: '',
...
type FilterItem = 'all' | CheckinItem
  ...
  const allEntries = useMemo<LogEntry[]>(() => {
    ...
    for (const c of checkins) {
      if (c.pushups && c.pushupsAt) {
        entries.push({ date: c.date, item: 'pushups', at: c.pushupsAt, count: c.pushupsCount, sets: c.pushupsSets })
      }
      if (c.squats && c.squatsAt) {
        entries.push({ date: c.date, item: 'squats', at: c.squatsAt, count: c.squatsCount, sets: c.squatsSets })
      }
      if (c.pullups && c.pullupsAt) {
        entries.push({ date: c.date, item: 'pullups', at: c.pullupsAt, count: c.pullupsCount, sets: c.pullupsSets })
      }
      if (c.plank && c.plankAt) {
        entries.push({ date: c.date, item: 'plank', at: c.plankAt, count: c.plankDuration, sets: c.plankSets })
      }
      if (c.kneeDrives && c.kneeDrivesAt) {
        entries.push({ date: c.date, item: 'kneeDrives', at: c.kneeDrivesAt, count: c.kneeDrivesCount, sets: c.kneeDrivesSets })
      }
      if (c.underlegClaps && c.underlegClapsAt) {
        entries.push({ date: c.date, item: 'underlegClaps', at: c.underlegClapsAt, count: c.underlegClapsCount, sets: c.underlegClapsSets })
      }
      ...
      if (c.pushups && !c.pushupsAt) {
        entries.push({ date: c.date, item: 'pushups', at: c.date + 'T00:00:00', count: c.pushupsCount, sets: c.pushupsSets })
      }
      if (c.squats && !c.squatsAt) {
        entries.push({ date: c.date, item: 'squats', at: c.date + 'T00:00:00', count: c.squatsCount, sets: c.squatsSets })
      }
      if (c.pullups && !c.pullupsAt) {
        entries.push({ date: c.date, item: 'pullups', at: c.date + 'T00:00:00', count: c.pullupsCount, sets: c.pullupsSets })
      }
      if (c.plank && !c.plankAt) {
        entries.push({ date: c.date, item: 'plank', at: c.date + 'T00:00:00', count: c.plankDuration, sets: c.plankSets })
      }
      if (c.kneeDrives && !c.kneeDrivesAt) {
        entries.push({ date: c.date, item: 'kneeDrives', at: c.date + 'T00:00:00', count: c.kneeDrivesCount, sets: c.kneeDrivesSets })
      }
      if (c.underlegClaps && !c.underlegClapsAt) {
        entries.push({ date: c.date, item: 'underlegClaps', at: c.date + 'T00:00:00', count: c.underlegClapsCount, sets: c.underlegClapsSets })
      }
      if (c.coldShower && !c.coldShowerAt) {
  ...
  return (
    <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-5">
      ...
      <div className="flex items-center gap-1.5 mb-4 flex-wrap">
        {(['all', 'pushups', 'squats', 'pullups', 'plank', 'kneeDrives', 'underlegClaps'] as FilterItem[]).map((f) => {
            ...
            <div key={date}>
              ...
              <div className="space-y-2">
                {entries.map((entry, idx) => {
                  ...
                  const unit = locale === 'zh' ? meta.unitZh : meta.unitEn
                  const setsLabel = entry.sets != null && entry.sets > 1 ? ` × ${entry.sets}${locale === 'zh' ? '组' : 'sets'}` : ''
                  return (
                    <div
                      key={`${entry.item}-${idx}`}
